...
export const getnwidth = ({commit, state}, payload) => {
    commit('getnwidth', payload)
}

export const getItemIndex = ({commit, state}, payload) => {
    commit('getItemIndex', payload)
}

export const maddItem = ({commit, state}, payload) => {
  let g = lib.guid()
  let k = { 'x': 0, 'y': 0, 'w': 4, 'h': 2, 'i': g, 'nw': 80 }
  commit('maddItem', k)
}
